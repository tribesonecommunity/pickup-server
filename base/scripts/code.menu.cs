...
function remoteMenuSelect(%clientId, %code)
{
   ...
   if(%clientId.cantUseYet)
   {
	%clientId.tries++;

	if(%clientId.tries == 10)
	{
		%clientId.beingKicked = true;
		Admin::Exploit(%clientId, "Menu Option Spam ("@%code@")", true);
   ...
   if(!%clientId.cantUseYet)
   {
	schedule(%clientId@".cantuseYet = false;",0.05);
	schedule(%clientId@".tries = 0;", 0.5);
	%clientId.cantUseYet = true;
   }
}
